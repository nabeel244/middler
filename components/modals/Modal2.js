...
const Modal2 = ({ onClose }) => {
  ...
  const canSend = emailValid && (choice.where || choice.why);
              ...
              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  {btns.map((btn) => (
                    <button
                      key={btn}
                      onClick={() => setChoice((c) => ({ ...c, [title]: btn }))}
                      className={`rounded-lg min-w-[135px] text-black p-2.5 border-2 border-white font-semibold transition-all duration-200 ease-in-out ${
                        choice[title] === btn
